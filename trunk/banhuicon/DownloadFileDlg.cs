...
namespace Banhuitong.Console {
    ...
    public partial class DownloadFileDlg : Form {
        ...
        public DownloadFileDlg(long fileId, string fileHash, string fileName, int fileLength) {
            InitializeComponent();
        ...
        private void backgroundWorker1_DoWork(object sender, System.ComponentModel.DoWorkEventArgs e) {
            ...
            using (var fs = new FileStream(FileName, FileMode.Create)) {
                if (FileLength > 0L) {
                    // 只下载长度大于0的文件。
                    UpdateProgress(FileLength, 0, 0D);

                    while (current < FileLength) {
                        var content = Files.Download(FileId, FileHash).Result;
                        if (content.Length > 0) {
                            ...
                            UpdateProgress(FileLength, current, elapsed);
