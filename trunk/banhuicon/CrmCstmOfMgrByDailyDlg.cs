...
namespace Banhuitong.Console {
    using Crm;
    using Excel;
    public partial class CrmCstmOfMgrByDailyDlg : Form {

        private string m_uname;
        public CrmCstmOfMgrByDailyDlg(string uname, DateTime time) {
            InitializeComponent();
            dtpDate.Value = time;
            m_uname = uname;
        }

        private void CrmCstmOfMgrByDailyDlg_Load(object sender, EventArgs e) {
        ...
        private async void UpdateTable() {
            ...
            var p = await CrmPerformance.GetCstmofMgrByDaily(d);
            if (p.IsOk) {
