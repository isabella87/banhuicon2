...
namespace Banhuitong.Console.Rpc {
    ...
    public static class Http {
        ...
        public static string BaseUrl {
            get {
                if (m_baseUrl == null) {
                    throw new InvalidOperationException("value of BaseUrl has not been set");
                } else {
                    return m_baseUrl;
                }
            }
        ...
        public static string FileUrl {
            get {
                if (m_fileUrl == null) {
                    throw new InvalidOperationException("value of FileUrl has not been set");
                } else {
                    return m_fileUrl;
                }
            }
            set {
                if (String.IsNullOrWhiteSpace(value)) {
                    ...
                    value = value.Trim();
                    m_fileUrl = value.EndsWith("/") ? value.MidStr(0, -1) : value;
                }
            }
        }
        /// <summary>
        ...
        /// <returns></returns>
        private static Uri NormalUri(string url, URLTYPES type = URLTYPES.VISIT) {
            url = Commons.NotBlank(url, "url").Trim();

            string fullUrl;
            if (type == URLTYPES.FILE)
                fullUrl = url.StartsWith("/") ? FileUrl + url : FileUrl + "/" + url;
            else
        ...
        /// <returns></returns>
        public static async Task<byte[]> GetRaw(string url, long start = 0L, long end = 0L, URLTYPES urlType = URLTYPES.VISIT) {
            var uri = NormalUri(url, urlType);
        ...
        public static async Task<byte[]> GetFileData(string url) {
            var uri = NormalUri(url, URLTYPES.FILE);
        ...
        public static async Task<IResult> PostFileData(string url, string filename, byte[] data) {
            var uri = NormalUri(url, URLTYPES.FILE);
            using (var formData = new MultipartFormDataContent()) {
                var fileContent = new ByteArrayContent(data);
                formData.Add(new StringContent("this is description"), "description", "description");
                ...
                using (var client = new HttpClient()) {
                    ...
                    if (!response.IsSuccessStatusCode) {
                        ...
                        return ToResult(response.Content.ReadAsStringAsync().Result);
                    }
                }
            }
        }

        public static async Task<IResult> DeleteFileData(string url) {
            var uri = NormalUri(url, URLTYPES.FILE);
            using (var client = new HttpClient()) {
        ...
        /// <returns></returns>
        public static async Task<IResult> Get(string url, IDictionary<string, object> data = null, URLTYPES urlType = URLTYPES.VISIT) {
            var uri = NormalUri(url, urlType);
            using (var client = new HttpWebClient(m_cookieContainer)) {
                m_status = HttpStatus.Receiving;
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        public static async Task<IResult> Put(string url, IDictionary<string, object> data = null, URLTYPES type = URLTYPES.VISIT) {
            return await Execute(url, "PUT", data, type);
        }
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        public static async Task<IResult> Delete(string url, IDictionary<string, object> data = null, URLTYPES type = URLTYPES.VISIT) {
            return await Execute(url, "DELETE", data, type);
        }
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        private static async Task<IResult> Execute(string url, string method, IDictionary<string, object> data = null, URLTYPES urlType = URLTYPES.VISIT) {
            var uri = NormalUri(url, urlType);
