...
namespace Banhuitong.Console {
    ...
    public partial class FilesDlg : Form {
        ...
        private async void UpdateTable() {
            ...
            r["object-id"] = m_objectId;
            r["file-type"] = m_fileType;
            if (m_pname != "") {
            ...
            if (p.IsOk) {
                ...
                foreach (var d in dl) {
                    var attributes = JArray.Parse(d["attributes"].ToStdString()).ToList();
                    //排除目录
                    if (attributes.IndexOf(16) != -1) {
                        continue;
                    }
                    ...
                    lvi.Tag = d["size"].ToInt32();
                    lvi.SubItems.Add(d["lastModifiedTime"].ToDateTime());
        ...
        private void btnDel_Click(object sender, EventArgs e) {
            btnDel.Enabled = false;
            var idArray = new List<Tuple<string, string, string>>();
            foreach (ListViewItem item in listView1.SelectedItems) {
                idArray.Add(Tuple.Create(item.SubItems[0].Text, item.SubItems[3].Text, item.SubItems[4].Text));
            }
            if (idArray.Count > 0)
                Del(idArray);
        }
