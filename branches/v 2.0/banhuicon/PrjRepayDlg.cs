...
namespace Banhuitong.Console {
    ...
    public partial class PrjRepayDlg : Form {
        ...
        private void myGridViewBinding2_GridDataTableSelectedRowChanged(object sender, MyLib.UI.MyGridDataTableSelectedRowChangedArgs e) {
            if (e.SelectedRowIndex.Length == 1) {
                var rowIndex = e.SelectedRowIndex[0];
                var paidAmt = Convert.ToDecimal(e.GetValue(rowIndex, "paidAmt"));
                if (paidAmt <= 0)
                    btnCancelRepay.Enabled = true;
                else
                    btnCancelRepay.Enabled = false;
        ...
        private async void CancelRepay() {
            btnCancel.Enabled = false;
            var r = await Projects.DeleteBonusDetail(m_pId, myGridViewBinding2.GetSelectedValues<long>("pbdId").FirstOrDefault());
            if (r.IsOk) {
                UpdateTable1();
                UpdateTable2();
            } else {
        ...
        private void btnCancelRepay_Click(object sender, EventArgs e) {
            CancelRepay();
        }
